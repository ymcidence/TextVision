...
def lstm_layer(tensor_in, out_length, num_layers, name='lstm'):
    def build_lstm(length):
        lstm_cell = tf.contrib.rnn.LSTMCell(length, initializer=tf.random_uniform_initializer(-0.1, 0.1),
                                            forget_bias=1.)
        return lstm_cell
